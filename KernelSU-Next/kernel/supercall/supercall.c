			  ...
			  void __user **arg)
{
	...
	/* ksud queries susfs through this reboot supercall (magic2 = SUSFS_MAGIC),
	 * not through prctl like the ksu_susfs CLI. Bridge it so the Manager can
	 * detect susfs and show its version/features. */
#define SUSFS_KSUD_MAGIC 0xFAFAFAFA
	if ((u32)magic2 == SUSFS_KSUD_MAGIC) {
		extern void susfs_handle_ksud_show(unsigned int cmd,
						   void __user *ubuf);
		susfs_handle_ksud_show(cmd, (void __user *)*arg);
		return 0;
	}
#endif
