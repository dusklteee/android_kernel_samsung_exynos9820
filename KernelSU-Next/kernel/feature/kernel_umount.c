...
#ifdef CONFIG_KSU_SUSFS_TRY_UMOUNT
/* Bridge for susfs, which calls ksu_try_umount() (the upstream KernelSU helper
 * name/signature). Map it onto KernelSU-Next's internal try_umount(). */
...
#ifdef CONFIG_KSU_SUSFS_TRY_UMOUNT
	/* susfs: umount user-defined paths first, before KSU's default umounts */
