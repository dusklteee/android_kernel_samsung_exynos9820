...
/*
 * SuSFS v2 allocates KSU mount ids from a dedicated high range so that they do
 * not collide with the kernel's own, which requires allocating from a minimum
 * on the very same IDA. That is the ida_alloc_min()/ida_free() API, introduced
 * after 4.14; ida_simple_get()/ida_simple_remove() are the equivalent here
 * (end == 0 means "no upper bound") and take care of their own locking, so the
 * rolling mnt_id_start/mnt_group_start hints and mnt_id_lock are gone.
 */
#define ida_alloc_min(ida, min, gfp)	ida_simple_get((ida), (min), 0, (gfp))
...
{
	int res = ida_alloc_min(&mnt_id_ida, 0, GFP_KERNEL);
